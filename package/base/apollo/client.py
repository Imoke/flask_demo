...
import requests
from package.base.config import config


class ApolloClient(object):
    def __init__(self, app_id, cluster='default', config_server_url='http://10.192.30.217:38080', timeout=35, ip=None):
    ...
    def get_value(self, key, default_val=None, namespace='application', auto_fetch_on_cache_miss=False):
        ...
        if config.get(namespace) != None and key in config[namespace]:
    ...
    def _cached_http_get(self, key, default_val, namespace='application'):
        url = '{}/configfiles/json/{}/{}/{}?ip={}'.format(
            self.config_server_url, self.appId, self.cluster, namespace, self.ip)
        r = requests.get(url)
        if r.ok:
            data = r.json()
